...
export default function ToursPage() {
  ...
  return (
    <div className="bg-brand-white dark:bg-brand-onyx min-h-screen pb-20 pt-20 transition-colors duration-300">
      {/* Hero Header */}
      <section className="bg-brand-onyx dark:bg-black py-24 text-white">
        <div className="container mx-auto px-4 text-center">
          ...
          >
            Signature <span className="text-brand-gold">Journeys</span>
          </motion.h1>
            ...
            >
              ...
              <div className="w-full lg:w-1/2 space-y-8">
                <div>
                  ...
                  <h2 className="text-4xl md:text-5xl font-heading font-bold text-brand-onyx dark:text-white mb-6">{service.title}</h2>
                ...
                <div className="grid grid-cols-2 gap-6 pb-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-xs text-gray-400 dark:text-gray-500 uppercase font-bold tracking-wider">Duration</p>
                      <p className="font-bold text-brand-onyx dark:text-white">{service.duration}</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-3">
                    <div className="h-10 w-10 bg-brand-gold/10 rounded-full flex items-center justify-center text-brand-gold">
                    ...
                    <div>
                      <p className="text-xs text-gray-400 dark:text-gray-500 uppercase font-bold tracking-wider">Experience</p>
                      <p className="font-bold text-brand-onyx dark:text-white">{service.type}</p>
                    </div>
                  </div>
                </div>
                ...
                <div className="flex flex-wrap gap-3">
                  {service.features.map((feature, i) => (
                    <span key={i} className="bg-gray-100 dark:bg-white/5 text-gray-500 dark:text-gray-400 px-4 py-2 rounded-full text-sm font-medium border border-gray-200 dark:border-white/10">
                      {feature}
                    </span>
                  ))}
                </div>
                ...
                <div className="pt-6 flex flex-col sm:flex-row items-center gap-8">
                  <div>
                    ...
                    <p className="text-4xl font-bold text-brand-onyx dark:text-white">{formatCurrency(service.price)}</p>
                  </div>
                  <Button 
                    onClick={() => handleAddToCart(service)}
                    size="lg" 
                    className="w-full sm:w-auto px-10 h-16 bg-brand-onyx dark:bg-brand-gold text-brand-white dark:text-brand-onyx hover:bg-brand-gold dark:hover:bg-brand-white hover:text-brand-onyx transition-all duration-300 rounded-2xl font-bold text-lg"
                  >
      ...
      <section className="bg-brand-onyx dark:bg-black mt-32 py-24 text-white overflow-hidden relative">
